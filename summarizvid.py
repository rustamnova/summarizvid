...
def build_multimodal_prompt(title: str, speech: str, screen_text: str, visual: str) -> str:
    ...
    return (
        ...
        "3. «Текст на экране» — только СОДЕРЖАТЕЛЬНЫЙ текст: титры, имена и "
        "должности, заголовки, названия, даты, цифры по теме, субтитры, "
        "адреса и ссылки. Случайные надписи с фона в кадре не переписывай: "
        "указатели («EXIT», «ВЫХОД»), номера секторов и рядов, вывески баров и "
        "магазинов, логотипы на технике и одежде, дорожные знаки — если они "
        "ничего не говорят о содержании видео, их место в блоке «Что показано» "
        "одной фразой про обстановку, а не списком. Каждую надпись упоминай "
        "один раз, повторы с разных кадров не дублируй. Блок целиком опусти, "
        "если содержательного текста нет.\n"
